...
test('home introduces the people and the club with working media',async({page})=>{
 await page.goto('/');
 await expect(page.getByRole('heading',{name:'Poczuj atmosferę WWFC.'})).toBeVisible();
 await expect(page.getByRole('img',{name:'Uczestnicy treningu na macie WWFC'})).toBeVisible();
 ...
 for(const image of await club.locator('img').all()){ await image.scrollIntoViewIfNeeded(); await expect.poll(()=>image.evaluate((img:HTMLImageElement)=>img.complete&&img.naturalWidth>0)).toBe(true); }
 await expect(page.locator('video')).toHaveCount(0);
 await page.getByRole('button',{name:'Odtwórz film: Trening w WWFC'}).click();
